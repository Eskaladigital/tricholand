...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: getAlternatesMetadata('it', 'shop', slug),
  }
}
    ...
    <>
      <article className="px-5 lg:px-8 py-16 max-w-6xl mx-auto">
        <nav className="text-sm text-marron-claro mb-6">
          <Link href="/it" className="hover:text-naranja transition-colors">Home</Link>
          <span className="mx-2">›</span>
          <Link href={getFullPath('it', 'shop')} className="hover:text-naranja transition-colors">Tienda B2B</Link>
          <span className="mx-2">›</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div>
            ...
            {product.variety_slug && (
              <Link
                href={getFullPath('it', 'varieties', product.variety_slug)}
            ...
            <div className="mt-6 mb-6 pb-6 border-b border-linea">
              ...
              <p className="text-xs text-marron-claro mt-1">Prezzo IVA esclusa · Passaporto fitosanitario incluso</p>

              {product.stock_qty !== null && (
                <p className={`text-sm mt-2 font-semibold ${isLowStock ? 'text-terracota' : 'text-verde'}`}>
                  {isLowStock
                    ? `⚡ Solo ${product.stock_qty} lotti disponibili`
                    : `● ${product.stock_qty} lotti disponibili`}
                </p>
              )}
            </div>
