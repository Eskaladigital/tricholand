...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${product.name} — B2B Winkel`,
    description: product.description.slice(0, 160),
    alternates: getAlternatesMetadata('nl', 'shop', slug),
  }
}
    ...
    <>
      <article className="px-5 lg:px-8 py-16 max-w-6xl mx-auto">
        <nav className="text-sm text-marron-claro mb-6">
          <Link href="/nl" className="hover:text-naranja transition-colors">Home</Link>
          <span className="mx-2">›</span>
          <Link href={getFullPath('nl', 'shop')} className="hover:text-naranja transition-colors">B2B Winkel</Link>
          <span className="mx-2">›</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div>
            ...
            {product.variety_slug && (
              <Link
                href={getFullPath('nl', 'varieties', product.variety_slug)}
                className="text-sm text-naranja font-semibold hover:underline"
              >
                Bekijk variëteitsfiche →
              </Link>
            )}
            ...
            <div className="mt-6 mb-6 pb-6 border-b border-linea">
              ...
              <p className="text-xs text-marron-claro mt-1">Prijs excl. BTW · Fytosanitair paspoort inbegrepen</p>

              {product.stock_qty !== null && (
                <p className={`text-sm mt-2 font-semibold ${isLowStock ? 'text-terracota' : 'text-verde'}`}>
                  {isLowStock
                    ? `⚡ Nog maar ${product.stock_qty} partijen beschikbaar`
                    : `● ${product.stock_qty} partijen beschikbaar`}
                </p>
              )}
            </div>
